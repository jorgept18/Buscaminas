...
public class VentanaBuscaminas extends javax.swing.JFrame {
    ...
    //declaro un arrayList para ir guardando la lista de botones que tengo que verificar
    ...
    //cuentaMinas realiza un paso previo que consiste en contar para cada celda el número
    //de minas que hay alrededor
    ...
    public VentanaBuscaminas() {
        ...
        for (int i=0; i<filas; i++){
            for (int j=0; j<columnas; j++){
                ...
                //añado el evento del click del ratón
                boton.addMouseListener(new MouseAdapter() {
                    @Override
                    public void mousePressed(MouseEvent evt){
                        //método a llamar cuando se pulse el botón
                        botonPulsado(evt);
    ...
    //metodo que cuando pulsas sobre un botón donde no haya bombas alrededor,
    //destapa el mismo y los que le rodean
    private void destapa(int i, int j){
         if ( j!=0 && i!=0 && j!=columnas-1 && i!=filas-1){
            arrayBotones[i-1][j].setEnabled(false);
            arrayBotones[i-1][j-1].setEnabled(false);
            arrayBotones[i-1][j+1].setEnabled(false);
            arrayBotones[i][j-1].setEnabled(false);
            arrayBotones[i][j+1].setEnabled(false);
            arrayBotones[i+1][j].setEnabled(false);
            arrayBotones[i+1][j+1].setEnabled(false);
            arrayBotones[i+1][j-1].setEnabled(false);
